...
export function StaffDetailClient({
  ...
  async function handleDelete() {
    if (!confirm(`¿Eliminar a ${staff.user.name}? Esta acción no se puede deshacer.`)) return;
    setDeleting(true);
    setDeleteError(null);
    try {
    ...
    <main className="p-6 space-y-6 max-w-3xl">
      ...
      {tab === "profile" && (
        <form onSubmit={saveProfile} className="space-y-4 bg-white rounded-2xl border border-gray-200 p-5">
          <div>
            ...
            <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={3} maxLength={500} className="input mt-1" placeholder="Descripción breve..." />
          </div>
          <div>
            <label className="label">Especialidades (separadas por coma)</label>
            <input type="text" value={specialties} onChange={(e) => setSpecialties(e.target.value)} className="input mt-1" placeholder="fade, corte clásico, barba" />
          </div>
          <div>
            <label className="label">Instagram URL</label>
            <input type="url" value={instagram} onChange={(e) => setInstagram(e.target.value)} className="input mt-1" placeholder="https://instagram.com/..." />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="label">Comisión (%)</label>
              <input type="number" min="0" max="100" step="0.5" value={commission} onChange={(e) => setCommission(e.target.value)} className="input mt-1" />
            </div>
            <div className="flex items-end pb-1">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={acceptsOnline} onChange={(e) => setAcceptsOnline(e.target.checked)} className="rounded" />
        ...
        <form onSubmit={saveHours} className="space-y-3 bg-white rounded-2xl border border-gray-200 p-5">
          ...
          {hours.map((h) => (
            <div key={h.dayOfWeek} className="flex items-center gap-3">
              <input type="checkbox" checked={h.isActive} onChange={(e) => updateHour(h.dayOfWeek, "isActive", e.target.checked)} className="rounded" />
              <span className="w-24 text-sm font-medium text-gray-700">{DAY_NAMES[h.dayOfWeek]}</span>
              <input type="time" value={h.startTime} disabled={!h.isActive} onChange={(e) => updateHour(h.dayOfWeek, "startTime", e.target.value)} className="input w-28 disabled:opacity-40" />
              <span className="text-gray-400 text-sm">a</span>
              <input type="time" value={h.endTime} disabled={!h.isActive} onChange={(e) => updateHour(h.dayOfWeek, "endTime", e.target.value)} className="input w-28 disabled:opacity-40" />
            </div>
          ))}
          ...
          <div className="space-y-2">
            {services.map((svc: any) => (
              <label key={svc.id} className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-xs text-gray-400 ml-auto">${svc.price?.toLocaleString("es-AR")} · {svc.durationMinutes}min</span>
              </label>
            ))}
          </div>
          <button onClick={saveServices} disabled={servicesSaving} className="btn-primary">
      ...
      {tab === "timeoff" && (
        <div className="space-y-4">
          <form onSubmit={addTimeOff} className="bg-white rounded-2xl border border-gray-200 p-5 space-y-3">
            <h3 className="font-semibold text-sm">Agregar franco / ausencia</h3>
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-3 sm:col-span-1">
                <label className="label">Fecha</label>
                <input type="date" required value={newDate} onChange={(e) => setNewDate(e.target.value)} className="input mt-1" />
              </div>
              <div>
                <label className="label">Desde (opcional)</label>
                <input type="time" value={newStartTime} onChange={(e) => setNewStartTime(e.target.value)} className="input mt-1" />
              </div>
              <div>
                <label className="label">Hasta (opcional)</label>
                <input type="time" value={newEndTime} onChange={(e) => setNewEndTime(e.target.value)} className="input mt-1" />
              </div>
            </div>
            <div>
              <label className="label">Motivo</label>
              <input type="text" value={newReason} onChange={(e) => setNewReason(e.target.value)} className="input mt-1" placeholder="Vacaciones, enfermedad, etc." />
            </div>
          ...
          <div className="space-y-2">
            ...
            {timeoffs.map((t) => (
              <div key={t.id} className={`rounded-xl border px-4 py-3 flex items-center justify-between gap-3 ${t.approved ? "border-green-200 bg-green-50" : "border-yellow-200 bg-yellow-50"}`}>
                <div>
                  <p className="text-sm font-medium">
                    {format(parseISO(t.date), "EEEE d 'de' MMMM yyyy", { locale: es })}
                    {t.startTime && t.endTime && (
                      <span className="text-gray-500 font-normal"> {t.startTime}–{t.endTime}</span>
                    )}
                  </p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {!t.approved ? (
                    <button onClick={() => approveTimeOff(t.id, true)} className="text-xs text-green-700 font-medium hover:underline">
                      Aprobar
                    </button>
                  ) : (
